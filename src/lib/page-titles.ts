...
export const CASE_STUDY_TITLES: Record<string, string> = {
  ...
  // "Personal Harrison's app" doesn't read as one phrase the way "Airbnb
  // account creation & onboarding" does.
  "/work/harrisons-app": "Harrison's app",
};
 ...
 * links in home.ts (`?p=<slug>`, or a gated `/work/<slug>` — see Harrison's
 * app), plus any main project whose case study is gated behind the
 * private-preview proxy (src/proxy.ts) — there `?p=` is the /work/<slug>
 * the public was redirected from. Returns the "Company | Title" breadcrumb
 * and which homepage section Back should land on, so PersistentHeader's
 * top bar and the page's own <title> never disagree; undefined for an
 * unknown slug. */
export function comingSoonProject(
  ...
  const small = additionalWork.find(
    (item) => item.href === `/coming-soon?p=${p}` || item.href === `/work/${p}`,
  );
  if (small) {
    return {
 ...
 * trigger on routes outside this set (lab pages). */
