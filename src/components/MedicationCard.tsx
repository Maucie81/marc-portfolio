...
import { getGivenAt, setGivenAt, unsetGivenAt } from "@/lib/storage";
import { scheduleNextDoseNotification } from "@/lib/notifications";
...
export function MedicationCard({ config, onCheckChange }: MedicationCardProps) {
  ...
  const doseRows = getDoseRowsForMedication(config, getGivenAtBound);

  const handleMarkGiven = (doseIndex: number) => {
    setGivenAt(config.id, doseIndex, new Date().toISOString());
    scheduleNextDoseNotification(config, doseIndex, getGivenAtBound);
    onCheckChange?.();
  };

  const handleUncheck = (doseIndex: number) => {
    unsetGivenAt(config.id, doseIndex);
    onCheckChange?.();
  };
      ...
      <ul className="mt-4 space-y-6">
        {doseRows.map((row) => (
          <li key={`${config.id}-${row.doseIndex}`}>
            <DoseRow
              row={row}
              asNeeded={config.asNeeded ?? false}
              onMarkGiven={() => handleMarkGiven(row.doseIndex)}
              onUncheck={row.canUncheck ? () => handleUncheck(row.doseIndex) : undefined}
            />
          </li>
        ))}
      </ul>
